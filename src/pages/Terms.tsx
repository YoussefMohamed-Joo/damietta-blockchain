          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
            {sections.map((s, i) => (
              <div key={s.num} style={{
                ...
                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#0F172A', marginBottom: '.5rem', marginTop: 0 }}>{t(`terms.s${s.num}_title`)}</h3>
                <p style={{ color: '#475569', fontSize: '.9rem', lineHeight: 1.8, margin: 0 }}>{t(`terms.s${s.num}_text`)}</p>
              </div>
            ))}
          </div>
        </div>
