...
function renderAuditLogs() {
  ...
  document.getElementById('audit-body').innerHTML = rows.length ? rows.map(l => `
    <tr>
      ...
      <td style="color:var(--text-dark);font-weight:500;">${esc(l.action)}</td>
    ...
    : `<tr><td colspan="7" class="text-center py-4" style="color:var(--text-faint);">No log entries match the current filters.</td></tr>`;
}
