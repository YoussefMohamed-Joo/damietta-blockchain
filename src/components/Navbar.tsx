    ...
    <header className="absolute top-0 left-0 right-0 z-20 pt-0 lg:pt-8 px-2 sm:px-6 lg:px-10">
      <nav className={`navbar-glass mx-auto max-w-5xl px-3 sm:px-4 py-0 flex items-center min-h-[20px] sm:min-h-[32px] lg:min-h-[58.5px] ${scrolled ? 'scrolled' : ''}`}>
        ...
        <ul className="hidden lg:flex items-center justify-center gap-6 xl:gap-8 flex-1" style={{ marginLeft: 60 }}>
          <li>{navLink('/', 'Home')}</li>
          <li>{navLink('/features', 'Features')}</li>
          <li>{navLink('/how-it-works', 'How It Works')}</li>
        </ul>
        <div className="hidden lg:flex items-center justify-end gap-2 xl:gap-3 shrink-0">
          <Link to="/login" className="login-btn px-4 xl:px-5 py-2.5 text-sm">Login</Link>
          <Link to="/register" className="register-btn px-4 xl:px-5 py-2.5 text-sm">Register</Link>
        </div>
        <button id="menuToggle" onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden ml-auto p-0.5 sm:p-1 rounded-lg hover:bg-slate-100 transition" aria-label="Toggle menu">
      ...
      <div className={`mx-auto max-w-5xl mt-2 navbar-glass mobile-menu ${menuOpen ? 'open' : ''}`}>
        <ul className="flex flex-col gap-4">
          <li><Link to="/" className={linkClass('/')} onClick={() => setMenuOpen(false)}>Home</Link></li>
          <li><Link to="/features" className={linkClass('/features')} onClick={() => setMenuOpen(false)}>Features</Link></li>
          <li><Link to="/how-it-works" className={linkClass('/how-it-works')} onClick={() => setMenuOpen(false)}>How It Works</Link></li>
        </ul>
        <div className="flex flex-col gap-3 mt-5 pt-5 border-t border-slate-200/60">
          <Link to="/login" className="login-btn w-full text-center px-5 py-2.5 text-sm" onClick={() => setMenuOpen(false)}>Login</Link>
          <Link to="/register" className="register-btn w-full text-center px-5 py-2.5 text-sm" onClick={() => setMenuOpen(false)}>Register</Link>
        </div>
      </div>
    </header>
  )
